...
struct _SynapticsPrivateRec {
    ...
    CARD32 three_finger_last_millis; /* Last timestamp when 3 fingers are on the trackpad in a 3-finger drag. It is possible to continue
                                        dragging using 1 or 2 fingers within the LockedDragTimeout limit after this time step for
                                        compatibility with the existing state transitions. However if we're in a 3-finger drag
                                        the user is only supposed to prolong the drag only with 3 fingers but not 1 or 2. 
    ...
    Bool has_seen_two_finger_scroll; // Whether we've seen a two-finger scroll in the current state transition loop
